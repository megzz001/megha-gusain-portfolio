...
export default function AboutMe() {
  ...
  const highlights = [
    ...
    },
    {
      ...
      title: "FinTech & Scalable Systems",
      desc: "Deeply interested in financial technology, high-throughput cloud environments, and implementing clean backend software architectures."
    }
  ];

  return (
    <section id="about" className="py-24 px-4 border-t border-neutral-100 dark:border-neutral-900 bg-neutral-50/40 dark:bg-neutral-900/10 transition-colors duration-300">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="lg:col-span-7 space-y-8">
            ...
            <div className="space-y-4 pt-4">
              <h3 className="text-sm font-mono uppercase tracking-wider text-neutral-400 dark:text-neutral-500">
                Core Specialties
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-1 gap-4">
                {highlights.map((item, idx) => (
                  <div 
                    key={idx} 
                    className="flex gap-4 p-5 rounded-xl border border-neutral-200/60 dark:border-neutral-800/60 bg-white dark:bg-brand-card-dark shadow-sm hover:border-brand-accent/30 dark:hover:border-brand-accent/30 transition-all duration-300"
                  >
                    <div className="flex-shrink-0 p-2.5 rounded-lg bg-neutral-100 dark:bg-neutral-800 h-11 w-11 flex items-center justify-center">
                      {item.icon}
                    </div>
                    <div>
            ...
            <div className="p-6 rounded-2xl border border-neutral-200 dark:border-brand-border-dark bg-white dark:bg-brand-card-dark shadow-sm">
              ...
              <div className="flex flex-wrap gap-2">
                {coursework.map((course, idx) => (
                  <span
                    key={idx}
                    className="px-3 py-1.5 rounded-lg bg-neutral-100 dark:bg-neutral-800/70 border border-neutral-200/50 dark:border-neutral-700/30 text-xs font-mono text-neutral-600 dark:text-neutral-300"
                  >
                    {course}
                  </span>
                ))}
            ...
            <div className="p-6 rounded-2xl border border-neutral-200 dark:border-brand-border-dark bg-white dark:bg-brand-card-dark shadow-sm">
              ...
              <ul className="space-y-3.5">
                {ME_INFO.additionalInfo.map((info, idx) => (
                  <li key={idx} className="flex gap-3 items-start text-sm text-neutral-600 dark:text-neutral-400">
